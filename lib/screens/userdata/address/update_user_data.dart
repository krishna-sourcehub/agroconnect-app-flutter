...
class backendServiceUpdateProfie {
  /// Backend login
  ...
  Future<Map<String, dynamic>> updateUPIId({
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse("$backendURL/user/updateUpIId"),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $sessionToken',
        },
      ...
      print("object $responseData");
      if (response.statusCode == 200) {
        await secureStorage.write(key: "UPIUpdateStatus", value: "true");
        return {'status': true, 'message':"UPI Id updated successfully"};
      }
      return {'status': false, 'message': 'server Error'};
